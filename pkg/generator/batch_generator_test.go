...
func TestBatchGenerator(t *testing.T) {
	...
	generateFunc := func(params *pkg.PerfParams, ns string, index int) (string, string) {
	...
	t.Run("no need to create ksvc", func(t *testing.T) {
		start := time.Now().Unix()
		generator.NewBatchGenerator(time.Duration(1)*time.Second, 0, 2, 2, []string{"ns1", "ns2"}, generateFunc, postGeneratorFunc, nil).Generate()
	...
	t.Run("should complete in 4s", func(t *testing.T) {
		start := time.Now().Unix()
		generator.NewBatchGenerator(time.Duration(1)*time.Second, 8, 2, 2, []string{"ns1", "ns2"}, generateFunc, postGeneratorFunc, nil).Generate()
		duration := time.Now().Unix() - start
	...
	t.Run("should complete in 2s", func(t *testing.T) {
		start := time.Now().Unix()
		generator.NewBatchGenerator(time.Duration(1)*time.Second, 8, 4, 2, []string{"ns1", "ns2"}, generateFunc, postGeneratorFunc, nil).Generate()
		duration := time.Now().Unix() - start
	...
	t.Run("should complete in 1s", func(t *testing.T) {
		start := time.Now().Unix()
		generator.NewBatchGenerator(time.Duration(1)*time.Second, 8, 8, 16, []string{"ns1", "ns2"}, generateFunc, postGeneratorFunc, nil).Generate()
		duration := time.Now().Unix() - start
